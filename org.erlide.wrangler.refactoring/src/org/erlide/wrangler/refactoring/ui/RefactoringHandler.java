...
public class RefactoringHandler extends AbstractHandler {
    ...
    public Object execute(final ExecutionEvent event) throws ExecutionException {
        ...
        if (actionId.equals("org.erlide.wrangler.refactoring.renamevariable")) {
            pages.add(new SimpleInputPage("Rename variable", "Please type the new variable name!", "New variable name:",
                    "New name must be a valid Erlang variable name!", new VariableNameValidator()));
            ...
            pages.add(new CostumworkFlowInputPage("Rename function", "Please type the new function name!", "New function name:",
                    "New name must be a valid Erlang atom!", new AtomValidator()));
            ...
            pages.add(new CostumworkFlowInputPage("Extract function", "Please type a function name!", "Function name:",
            ...
            pages.add(new CostumworkFlowInputPage("Rename module", "Please type the new module name!", "New module name:",
                    "New module name must be a valid Erlang atom!", new AtomValidator()));
            ...
            IProject project = GlobalParameters.getWranglerSelection().getErlElement().getErlProject().getProject();
            ArrayList<String> moduleList = WranglerUtils.getModuleNames(project);
            String moduleName = GlobalParameters.getWranglerSelection().getErlElement().getResource().getName();
            ...
            pages.add(new SelectionInputPage("Fold expression", "Please select expression which should be fold!", "Select expressions which should be folded!",
                    (CostumWorkflowRefactoringWithPositionsSelection) refactoring));

            // run fold expression against a remote function
        } else if (actionId.equals("org.erlide.wrangler.refactoring.introducenewvariable")) {
            refactoring = new IntroduceNewVariableRefactoring();

            pages.add(new SimpleInputPage("Introduce new variable", "", "Please insert variable's name", "Variable name is incorrect",
            ...
            if (actionId.equals("org.erlide.wrangler.refactoring.foldremoteexpression")) {
                ...
                // dialog
                // may change it
                IErlMemberSelection sel = (IErlMemberSelection) GlobalParameters.getWranglerSelection();

                RemoteFunctionClauseDialog dialog = new RemoteFunctionClauseDialog(activeShell, "Fold expression");
                ...
                if (dialog.isFinished()) {
                    ...
                    pages.add(new SelectionInputPage("Fold expression", "Please select expression which should be fold!",
                            "Select expressions which should be folded!", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));
                ...
                pages.add(new SimpleInputPage("Introduce macro definition", "Please type the new macro name!", "New macro name:",
                ...
                pages.add(new SimpleInputPage("Rename process", "Please type the new process name!", "New process name:",
                        "New process name must be an Erlang atom!", new AtomValidator()));

                // run function to process refactoring
            } else if (actionId.equals("org.erlide.wrangler.refactoring.functiontoprocess")) {
                refactoring = new FunctionToProcessRefactoring();
                pages.add(new SimpleInputPage("Convert function to process", "Please type the new process name!", "New process name:",
                        "New process name must be an Erlang atom!", new AtomValidator()));

                // run tuple function parameters refactoring
            } else if (actionId.equals("org.erlide.wrangler.refactoring.tuplefunctonparameters")) {
            ...
            } else if (actionId.equals("org.erlide.wrangler.refactoring.foldagainstmacro")) {
                refactoring = new FoldAgainstMacro();

                pages.add(new SelectionInputPage("Fold against macro definition", "Please select expression which should be fold!",
                        "Select expressions which should be folded!", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));

                // normalize record expression
            } else if (actionId.equals("org.erlide.wrangler.refactoring.normalizerecordexpression")) {
                boolean showDefaultFields = MessageDialog.openQuestion(activeShell, "Showing defaults", "Show record fields with default values?");
                ...
                pages.add(new CostumworkFlowInputPage("Introduce ?LET", "Please type the pattern variable name!", "Pattern variable name:",
                        "New name must be a valid Erlang atom!", new VariableNameValidator()));
                ...
                pages.add(new SelectionInputPage("Merge ?LET expressions", "Please select expressions which whould be merged!",
                        "Select expressions which should be merged", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));
            } else if (actionId.equals("org.erlide.wrangler.refactoring.mergeforall")) {
                refactoring = new MergeForAllRefactoring();
                pages.add(new SelectionInputPage("Merge ?FORALL expressions", "Please select expressions which should be merged!",
                        "Select expressions which should be merged", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));

            } else if (actionId.equals("org.erlide.wrangler.refactoring.eqc_statemtorecord")) {
            ...
            } else if (actionId.equals("org.erlide.wrangler.refactoring.unfoldfunctionapplication")) {
                refactoring = new UnfoldFunctionApplicationRefactoring();

            } else if (actionId.equals("org.erlide.wrangler.refactoring.partitionexports")) {
                refactoring = new PartitionExportsRefactoring();
                SimpleInputPage page = new SimpleInputPage("Partition exports", "Please input the the distance treshould between 0.1 and 1.0",
                        "Distance treshold", "The value must be between 0.1 and 1.0", new NormalDoulbeValidator());
                page.setInput("0.8");
                pages.add(page);
            }
    ...
    protected void checkWarningMessages() {
        try {
            RpcResult res = WranglerBackendManager.getRefactoringBackend().getLoggedInfo();
            ...
            if (res.isOk()) {
                ...
                if (!resobj.equals(new OtpErlangList())) {
                    ...
                    for (int i = 0; i < reslist.arity(); ++i) {
                        OtpErlangTuple restuple = (OtpErlangTuple) reslist.elementAt(i);
                        OtpErlangString msg = (OtpErlangString) restuple.elementAt(1);
                        String formattedString = formatWarningString(msg.stringValue());
     ...
     */
    protected WranglerRefactoring runGenFunRefactoring(final ArrayList<WranglerPage> pages, final Shell activeShell) throws OtpErlangRangeException {
        WranglerRefactoring refactoring = null;

        IErlMemberSelection sel = (IErlMemberSelection) GlobalParameters.getWranglerSelection();

        // Ask the user about a new name
        NewParameterNameInputDialog dialog = new NewParameterNameInputDialog(activeShell, "New parameter name");
        dialog.open();
        if (!dialog.isFinished())
            return null;
        ...
        // call initial RPC
        GenFunRefactoringMessage m = (GenFunRefactoringMessage) WranglerBackendManager.getRefactoringBackend().callWithParser(new GenFunRefactoringMessage(),
                "generalise_eclipse", "sxxsxi", sel.getFilePath(), sel.getSelectionRange().getStartPos(), sel.getSelectionRange().getEndPos(), newParamName,
        ...
        if (m.getRefactoringState() == RefactoringState.OK) {
            ...
            pages.add(new SelectionInputPage("Generalise expression", "Please select which of them should be generalised!",
                    "Select one of them, which should be folded!", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));

        } else if (m.getRefactoringState() == RefactoringState.MORE_THAN_ONE_CLAUSE) {
            boolean selectedClauseOnly = MessageDialog.openQuestion(activeShell, "Multiple clauses",
            ...
            boolean sideEffect = MessageDialog.openQuestion(activeShell, "Side effect", "Does the expression selected has side effect?");

            OtpErlangObject noOfClausesPar = m.getParameters().get(GenFunReturnParameterName.noOfClauses);
            if (noOfClausesPar != null && ((OtpErlangLong) noOfClausesPar).intValue() > 1) {
                ...
                if ((!selectedClauseOnly && ((OtpErlangList) m.getParameters().get(GenFunReturnParameterName.dupsInFun)).arity() > 0)
                        || (selectedClauseOnly && ((OtpErlangList) m.getParameters().get(GenFunReturnParameterName.dupsInClause)).arity() > 0))
                    pages.add(new SelectionInputPage("Generalise expression", "Please select which of them should be generalised!",
                            "Select one of them, which should be folded!", (CostumWorkflowRefactoringWithPositionsSelection) refactoring));
