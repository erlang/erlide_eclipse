...
public class GeneraliseFunctionRefactoring extends
	...
	public IRefactoringRpcMessage run(IErlSelection sel) {
		...
		if (this.state == State.ok)
			...
			if (this.state == State.multi_instance) {
				...
				if (onlyInClause)
					...
					return WranglerBackendManager.getRefactoringBackend().call(
							...
							sel.getSearchPath(),
							GlobalParameters.getTabWidth(), logCmd);
			} else if (this.state == State.more_than_one_clause) {
				if (onlyInClause)
					return WranglerBackendManager.getRefactoringBackend().call(
							"gen_fun_clause_eclipse", "sxxxxxixxx",
							sel.getFilePath(), parName, funName, arity,
