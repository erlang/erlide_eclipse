...
public class SampledQuantile {
     ...
     */
    public SampledQuantile(final int maxSamples) {
     ...
     * @param units
     *            indication of what time units windowLength is specified in
     */
    public SampledQuantile(final long windowLength, final TimeUnit units) {
     ...
     *            indication of what time units windowLength is specified in
     */
    public SampledQuantile(final int maxSamples, final long windowLength,
    ...
    private void setWindowMillis(final long windowLength, final TimeUnit units) {
        switch (units) {
        case NANOSECONDS:
            windowMillis = windowLength / 1000000;
            break;
        case MICROSECONDS:
            windowMillis = windowLength / 1000;
            break;
        case MILLISECONDS:
            windowMillis = windowLength;
            break;
        case SECONDS:
            windowMillis = windowLength * 1000;
            break;
     ...
     * @throws QuantileOutOfBoundsException
     *             if i <= 0 or i >= 100
     */
    public long getPercentile(final int i) {
        return getPercentile(i, System.currentTimeMillis());
    }

    long getPercentile(final int i, final long now) {
     ...
     *             if k <= 0 or k >= q
     */
    public long getQuantile(final int k, final int q) {
        return getQuantile(k, q, System.currentTimeMillis());
    }

    long getQuantile(final int k, final int q, final long now) {
        if (k <= 0 || k >= q) {
            throw new QuantileOutOfBoundsException();
        }
        final List<Sample> validSamples = getValidSamples(now);
        if (validSamples.size() == 0) {
            return 0;
        }
        Collections.sort(validSamples);
        final double targetIndex = validSamples.size() * k / (q * 1.0);
        if (validSamples.size() % 2 == 1) {
            return validSamples.get((int) Math.ceil(targetIndex) - 1).data;
        }
        final int i0 = (int) Math.floor(targetIndex) - 1;
        return (validSamples.get(i0).data + validSamples.get(i0 + 1).data) / 2;
    }

    private List<Sample> getValidSamples(final long now) {
    ...
    /**
     * Reports a sample measurement to be included in the quantile calculations.
     *
     * @param l
     *            specific measurement
     */
    public void addSample(final long l) {
        addSample(l, System.currentTimeMillis());
    }

    private synchronized void updateWindowSegments(final long now) {
        if (windowMillis == null) {
            return;
        }
        final long deadline = now - windowMillis;
        ...
        final long mostRecentSegmentTimestamp = windowSegments.size() > 0
    ...
    private long getEffectiveSamplesSeen() {
        if (windowMillis == null) {
            return samplesSeen.get();
        }
        return samplesSeen.get() - windowSegments.getFirst().data;
    }

    void addSample(final long l, final long now) {
        ...
        if (rand.nextDouble() < maxSamples * 1.0 / getEffectiveSamplesSeen()) {
            final int idx = rand.nextInt(maxSamples);
    ...
    private static class Sample implements Comparable<Sample> {
        ...
        public Sample(final long data, final long timestamp) {
            this.data = data;
