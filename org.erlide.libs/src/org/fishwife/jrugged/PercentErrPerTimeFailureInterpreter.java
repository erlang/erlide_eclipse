...
public final class PercentErrPerTimeFailureInterpreter implements FailureInterpreter {
     ...
     */
    @SuppressWarnings("unchecked")
    public PercentErrPerTimeFailureInterpreter(final RequestCounter rc, final int percent,
     ...
     */
    @SuppressWarnings("unchecked")
    public PercentErrPerTimeFailureInterpreter(final PerformanceMonitor p,
            final int percent, final long windowMillis) {
        setIgnore(defaultIgnore);
        setPercent(percent);
        setWindowMillis(windowMillis);
     ...
     *            length of the window in milliseconds
     */
    public PercentErrPerTimeFailureInterpreter(final RequestCounter rc,
            final Class<? extends Throwable>[] ignore, final int percent,
            final long windowMillis) {
        setRequestCounter(rc);
        setIgnore(ignore);
        setPercent(percent);
    ...
    private boolean hasWindowConditions() {
        return percent > 0 && windowMillis > 0;
    }

    @Override
    public boolean shouldTrip(final Throwable cause) {
        if (isExceptionIgnorable(cause)) {
            return false;
        }
        ...
        if (hasWindowConditions()) {
            ...
            synchronized (modificationLock) {
                ...
                final long removeTimeBeforeMillis = System.currentTimeMillis()
                        - windowMillis;
                final int numberOfErrorsBefore = errorTimes.size();

                removeErrorsPriorToCutoffTime(numberOfErrorsBefore,
                        removeTimeBeforeMillis);

                numberOfErrorsAfter = errorTimes.size();

                currentRequestCount = requestCounts.get(requestCounts.size() - 1);
            }

            final long windowRequests = currentRequestCount
                    - previousRequestHighWaterMark;

            // Trip if the number of errors over the total of requests over the
            // same period
            // is over the percentage limit.
            return windowRequests >= requestThreshold && (double) numberOfErrorsAfter
                    / (double) windowRequests * 100d >= percent;
        }
        return true;
    }
    ...
    private void removeErrorsPriorToCutoffTime(final int numberOfErrorsBefore,
            final long removeTimeBeforeMillis) {
        boolean windowRemoval = false;

        // (could we speed this up by using binary search to find the entry
        // point,
        // then removing any items before that point?)
        for (int j = numberOfErrorsBefore - 1; j >= 0; j--) {

            final Long time = errorTimes.get(j);
     ...
     */
    public void setRequestCounter(final RequestCounter rc) {
        requestCounter = rc;
    }
     ...
     */
    public void setRequestThreshold(final int requestThreshold) {
