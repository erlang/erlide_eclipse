...
public final class PresenterControlCreator extends AbstractReusableInformationControlCreator {
	...
	private static final class BackAction extends Action {
		private final ErlangBrowserInformationControl fInfoControl;

		public BackAction(final ErlangBrowserInformationControl infoControl) {
		...
		@Override
		public void run() {
			final ErlangBrowserInformationControlInput current = fInfoControl.getInput();
			final ErlangBrowserInformationControlInput previous = current.getPrevious();
			if (previous != null) {
	...
	private static final class ForwardAction extends Action {
		...
		public ForwardAction(final ErlangBrowserInformationControl infoControl) {
			fInfoControl = infoControl;
			setText("Next");
			final ISharedImages images = PlatformUI.getWorkbench().getSharedImages();
		...
		public void run() {
			final ErlangBrowserInformationControlInput current = fInfoControl.getInput();
			final ErlangBrowserInformationControlInput next = current.getNext();
			if (next != null) {
	...
	private static final class ShowInEdocViewAction extends Action {
		...
		public ShowInEdocViewAction(final ErlangBrowserInformationControl infoControl) {
			fInfoControl = infoControl;
		...
		@Override
		public void run() {
			final ErlangBrowserInformationControlInput input = fInfoControl.getInput();
	...
	private static final class OpenDeclarationAction extends Action {
		...
		public OpenDeclarationAction(final ErlangBrowserInformationControl infoControl,
		...
		@Override
		public void run() {
			final ErlangBrowserInformationControlInput infoInput = fInfoControl.getInput();
			fInfoControl.notifyDelayedInputChange(null);
			fInfoControl.dispose();
	...
	protected IInformationControl doCreateInformationControl(final Shell parent) {
		if (ErlangBrowserInformationControl.isAvailable(parent)) {
			...
			final ErlangBrowserInformationControl control = new ErlangBrowserInformationControl(parent, font, tbm);

			final PresenterControlCreator.BackAction backAction = new PresenterControlCreator.BackAction(control);
			...
			if (editor != null) {
				final OpenDeclarationAction openDeclarationAction = new OpenDeclarationAction(control, editor);
