...
public final class BackendManager implements IBackendManager {
	...
	private final Set<@NonNull IBackend> allBackends;
	...
	public BackendManager(final IBackendFactory factory) {
		...
		launchListener = new BackendManagerLaunchListener(this, DebugPlugin.getDefault().getLaunchManager());
	}

	@Override
	public IBackend createExecutionBackend(final BackendData data) {
		ErlLogger.debug("create execution backend " + data.getNodeName());
		final @NonNull IBackend b = factory.createBackend(data);
		addBackend(b);
		notifyBackendChange(b, BackendEvent.ADDED, null, null);
		return b;
	}

	private void addBackend(final @NonNull IBackend b) {
	...
	void notifyBackendChange(final IBackend b, final BackendEvent type, final IProject project,
	...
	@Override
	public synchronized void updateNodeStatus(final String host, final Collection<String> started,
	...
	private void addBundle(final Bundle b, final RuntimeVersion version, final Multimap<CodeContext, String> paths,
	...
	@Override
	public Collection<@NonNull IBackend> getAllBackends() {
		synchronized (allBackends) {
	...
	@Override
	public void moduleLoaded(final IBackend b, final IProject project, final String moduleName) {
		notifyBackendChange(b, BackendEvent.MODULE_LOADED, project, moduleName);
	}

	@Override
	public synchronized Set<IBackend> getExecutionBackends(@NonNull final IProject project) {
