/*******************************************************************************
 * Copyright (c) 2010 Vlad Dumitrescu and others. All rights reserved. This program and
...
public class OtpNodeProxy implements IOtpNodeProxy {
    ...
    private boolean pingPeer() {
        ...
        boolean ok = false;
        while (!ok && tries > 0) {
            ok = localNode.ping(getNodeName(), OtpNodeProxy.RETRY_DELAY
                    + (OtpNodeProxy.MAX_RETRIES - tries) * OtpNodeProxy.RETRY_DELAY % 3);
            tries--;
        }
        return ok;
    }
