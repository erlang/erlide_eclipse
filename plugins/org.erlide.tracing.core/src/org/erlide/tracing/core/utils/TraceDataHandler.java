...
public class TraceDataHandler {
     ...
     * @param message
     *            message
     * @return <code>true</code> if it is last message, <code>false</code> otherwise
     */
    public boolean isTracingFinished(final OtpErlangObject message) {
        if (message instanceof OtpErlangAtom) {
            ...
            if (atom.atomValue().equals(TraceDataHandler.ATOM_STOP_TRACING)) {
                return true;
            }
        }
        return false;
     ...
     * @param message
     *            message
     * @return error reason or <code>null</code> or if it is not an error message
     */
    public OtpErlangObject getErrorReson(final OtpErlangObject message) {
        if (message instanceof OtpErlangTuple) {
            ...
            if (tuple.elementAt(0) instanceof OtpErlangAtom) {
                ...
                if (atom.atomValue().equals(TraceDataHandler.ATOM_ERROR_LOADING)) {
    ...
    /**
     * Reads data described by given object. It can be trace event or information about
     * file containing tracing results.
     *
            ...
            if (otpErlangObject instanceof OtpErlangTuple) {
                ...
                if (atomValue.equals(TraceDataHandler.ATOM_TRACE_TS)) {
                    ...
                    final OtpErlangAtom traceType = (OtpErlangAtom) tuple
                            .elementAt(TraceDataHandler.INDEX_TRACE_TYPE);
                    lastTraceDate = readDateTuple(
                            (OtpErlangTuple) tuple.elementAt(tuple.arity() - 1));
    ...
    /**
     * Converts tuple representing date and time (<code>{date(), time()}</code>) to java
     * {@link Date} object.
     *
     * @param tuple
     *            date tuple
    ...
    private String pid2Str(final OtpErlangPid pid) {
        return String.valueOf(pid.id()) + "." + pid.serial() + "." + pid.creation();
    }
    ...
    private ITreeNode processFileInfo(final OtpErlangTuple tuple) {
        ...
        if (tuple.elementAt(
                TraceDataHandler.INDEX_INFO_START_DATE) instanceof OtpErlangAtom) {
            // file contains no trace events
            return null;
        }
        try {
            final Date from = readDateTuple((OtpErlangTuple) tuple
                    .elementAt(TraceDataHandler.INDEX_INFO_START_DATE));
            final Date to = readDateTuple((OtpErlangTuple) tuple
                    .elementAt(TraceDataHandler.INDEX_INFO_END_DATE));
            final String path = ((OtpErlangString) tuple
                    .elementAt(TraceDataHandler.INDEX_INFO_PATH)).stringValue();
            final long size = ((OtpErlangLong) tuple
                    .elementAt(TraceDataHandler.INDEX_INFO_COUNT)).longValue();
    ...
    private ITreeNode createProcessNode(final String label,
        ...
        if (erlangObject instanceof OtpErlangTuple) {
            ...
            if (processTuple.arity() == 3) {
                ...
                pid = (OtpErlangPid) processTuple
                        .elementAt(TraceDataHandler.INDEX_PROCESS_PID);
            ...
            final OtpErlangObject info = processTuple
                    .elementAt(TraceDataHandler.INDEX_PROCESS_INFO - index);

            // process node
            final OtpErlangObject processNode = processTuple
                    .elementAt(TraceDataHandler.INDEX_PROCESS_NODE - index);
    ...
    private ITreeNode createFunctionNode(final String label,
        ...
        if (erlangObject instanceof OtpErlangTuple) {
            ...
            final OtpErlangAtom moduleName = (OtpErlangAtom) functionTuple
                    .elementAt(TraceDataHandler.INDEX_FUNCTION_MODULE);
            final OtpErlangAtom functionName = (OtpErlangAtom) functionTuple
                    .elementAt(TraceDataHandler.INDEX_FUNCTION_NAME);
            ...
            final OtpErlangObject arityOrArgs = functionTuple
                    .elementAt(TraceDataHandler.INDEX_FUNCTION_ARGS);
            int arityValue = -1;
            if (arityOrArgs instanceof OtpErlangList) {
                ...
                try {
                    if (functionTuple.elementAt(
                            TraceDataHandler.INDEX_FUNCTION_ARGS) instanceof OtpErlangInt) {
    ...
    private ITreeNode processGcTrace(final String label, final Images image,
        ...
        final ITreeNode processNode = createProcessNode("process: ",
                tuple.elementAt(TraceDataHandler.INDEX_PROCESS));
        ...
        final OtpErlangList list = (OtpErlangList) tuple
                .elementAt(TraceDataHandler.INDEX_INFO);
    ...
    private ITreeNode processExceptionFrom(final String label,
        ...
        final OtpErlangTuple exceptionTuple = (OtpErlangTuple) tuple
                .elementAt(TraceDataHandler.INDEX_EXCEPTION);
        final OtpErlangObject exceptionClass = exceptionTuple
                .elementAt(TraceDataHandler.INDEX_EXCEPTION_CLASS);
        final OtpErlangObject exceptionValue = exceptionTuple
                .elementAt(TraceDataHandler.INDEX_EXCEPTION_VALUE);
    ...
    private ITreeNode processReturnTrace(final String label, final Images image,
        ...
        final ITreeNode functionNode = createFunctionNode("function: ",
                tuple.elementAt(TraceDataHandler.INDEX_FUNCTION));
        functionNode.setImage(Activator.getImage(Images.FUNCTION_NODE));
        ...
        if (showRetValue) {
            final ITreeNode returnValueNode = new TreeNode("return value: "
                    + tuple.elementAt(TraceDataHandler.INDEX_RETURN_VALUE));
            returnValueNode.setImage(Activator.getImage(Images.INFO_NODE));
            labelBuilder.append("->")
                    .append(tuple.elementAt(TraceDataHandler.INDEX_RETURN_VALUE));
    ...
    private ITreeNode processRegisterTrace(final String label, final Images image,
        ...
        final TreeNode regName = new TreeNode(
                "name: " + tuple.elementAt(TraceDataHandler.INDEX_REGNAME).toString(),
                Activator.getImage(Images.INFO_NODE));
    ...
    private ITreeNode processExitTrace(final String label, final OtpErlangTuple tuple) {
        ...
        final ITreeNode reasonNode = new TreeNode(
                "reason: " + tuple.elementAt(TraceDataHandler.INDEX_REASON).toString(),
                Activator.getImage(Images.INFO_NODE));
    ...
    private ITreeNode processReceiveTrace(final String label,
        ...
        final ITreeNode messageNode = createMessageNode(
                tuple.elementAt(TraceDataHandler.INDEX_MESSAGE));
    ...
    private ITreeNode processSendTrace(final String label, final Images image,
        ...
        final ITreeNode receiverNode = createProcessNode("receiver: ",
                tuple.elementAt(TraceDataHandler.INDEX_TO));
