 ...
 */
public class ErlangEditor extends AbstractErlangEditor implements IOutlineContentCreator, IOutlineSelectionHandler {
	...
	private ClearAllCachesAction clearAllCachesAction;
	private CallHierarchyAction callhierarchy;

	private final AnnotationSupport annotationSupport;

	final MarkOccurencesSupport markOccurencesHandler = new MarkOccurencesSupport(this, null, IDocumentExtension4.UNKNOWN_MODIFICATION_STAMP);
	...
	public void dispose() {
		...
		ErlideUIPlugin.getDefault().getPreferenceStore().removePropertyChangeListener(propertyChangeListener);
	...
	public static IPreferenceStore getErlangEditorPreferenceStore() {
		...
		return new ChainedPreferenceStore(new IPreferenceStore[] { ErlideUIPlugin.getDefault().getPreferenceStore(), generalTextStore });
	}
	...
	protected void addFoldingSupport(final ISourceViewer viewer) {
		if (EditorUtility.isFoldingEnabled()) {
			...
			fProjectionModelUpdater = ErlideUIPlugin.getDefault().getFoldingStructureProviderRegistry().getCurrentFoldingProvider();
			if (fProjectionModelUpdater != null) {
	 ...
	 */
	protected void synchronizeOutlinePage(final ISourceReference element, final boolean checkIfOutlinePageActive) {
		if (myOutlinePage != null) {
	...
	public void createPartControl(final Composite parent) {
		...
		ErlideUIPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(propertyChangeListener);

		PlatformUI.getWorkbench().addWindowListener(markOccurencesHandler.fActivationListener);
	...
	private IllegalArgumentException filterUTF8Exception(final IllegalArgumentException e) {
		...
		if (filterIt) {
			return new IllegalArgumentException("The file's actual encoding doesn't match the declared one", e);
		}
		return e;
	}
	...
	public void expandCollapseFunctionsOrComments(final boolean collapse, final boolean comments) {
