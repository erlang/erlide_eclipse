...
public class SendToConsoleAction extends SelectionDispatchAction {
	...
	public void run(final ITextSelection selection0) {
		...
		if (project != null) {
			...
			if (console == null) {
				...
				ErrorDialog.openError(getShell(), "No runtime", message,
			...
			if (getOutput) {
				consoleBackendShellListener = new ConsoleBackendShellListener(console.getShell(),
						getLineSelection(selection, true).getOffset());
	...
	public void addMessage(final int offset, final String message) {
		...
		try {
			final String delimiter = document.getLineDelimiter(document.getLineOfOffset(offset - 1));
			String nl = "";
			if (delimiter == null) {
	...
	public SendToConsoleAction(final IWorkbenchSite site, final ResourceBundle bundle, final String prefix,
			final ITextEditor editor, final boolean getOutput, final IProject project) {
		super(site);
