...
public class OtpCookedConnection extends AbstractConnection {
	...
	/*
	 * The connection needs to know which local pids have links that pass through
	 * here, so that they can be notified in case of connection failure
	 */
	 ...
	 */
	// package scope
	OtpCookedConnection(final OtpNode self, final OtpTransport s) throws IOException, OtpAuthException {
		super(self, s);
		this.self = self;
		links = new Links(25);
		start();
	 ...
	 */
	// package scope
	OtpCookedConnection(final OtpNode self, final OtpPeer other) throws IOException, OtpAuthException {
		super(self, other);
		this.self = self;
		links = new Links(25);
		start();
	 ...
	 * itself needs to know about links (in case of connection failure), so we snoop
	 * for link/unlink too here.
	 */
	@Override
	...
	void send(final OtpErlangPid from, final OtpErlangPid dest, final OtpErlangObject msg) throws IOException {
	 ...
	 */
	@SuppressWarnings("resource")
	void send(final OtpErlangPid from, final String dest, final OtpErlangObject msg) throws IOException {
		// encode and send the message
		sendBuf(from, dest, new OtpOutputStream(msg));
	}
	 ...
	 */
	void exit(final OtpErlangPid from, final OtpErlangPid to, final OtpErlangObject reason) {
		try {
	 ...
	 */
	void exit2(final OtpErlangPid from, final OtpErlangPid to, final OtpErlangObject reason) {
		try {
	 ...
	 */
	synchronized void link(final OtpErlangPid from, final OtpErlangPid to) throws OtpErlangExit {
		try {
	...
	/*
	 * When the connection fails - send exit to all local pids with links through
	 * this connection
	 */
		...
		if (links != null) {
			...
			if (l != null) {
				for (final Link aL : l) {
					...
					self.deliver(
							new OtpMsg(OtpMsg.exitTag, aL.remote(), aL.local(), new OtpErlangAtom("noconnection")));
				}
			}
		}
	}
