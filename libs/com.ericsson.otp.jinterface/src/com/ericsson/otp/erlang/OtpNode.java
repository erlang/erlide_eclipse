...
public class OtpNode extends OtpLocalNode {
	 ...
	 *
	 */
	public OtpNode(final String node, final OtpTransportFactory transportFactory) throws IOException {
	 ...
	 *
	 */
	public OtpNode(final String node, final String cookie, final OtpTransportFactory transportFactory)
			throws IOException {
	 ...
	 *
	 */
	public OtpNode(final String node, final String cookie, final int port) throws IOException {
		super(node, cookie);
	 ...
	 *
	 */
	public OtpNode(final String node, final String cookie, final int port, final OtpTransportFactory transportFactory)
			throws IOException {
	...
	/**
	 * Close the node. Unpublish the node from Epmd (preventing new connections) and
	 ...
	 *
	 *            <p>
	 *            If there are links from the mailbox to other {@link OtpErlangPid
	 *            pids}, they will be broken when this method is called and exit
	 *            signals with reason 'normal' will be sent.
	 *            </p>
	 *
	 */
	 ...
	 *            If there are links from the mailbox to other {@link OtpErlangPid
	 *            pids}, they will be broken when this method is called and exit
	 *            signals with the given reason will be sent.
	 *            </p>
	 *
	 */
	...
	/**
	 * Create an named mailbox that can be used to send and receive messages with
	 * other, similar mailboxes and with Erlang processes. Messages can be sent to
	 * this mailbox by using its registered name or the associated
	 * {@link OtpMbox#self() pid}.
	 *
	 * @param name
	 ...
	 * {@link OtpErlangPid pid} of the mailbox. A mailbox can have at most one name;
	 ...
	 *
	 * @return an array of Strings, containins all known registered names on this
	 *         node.
	 */
	 ...
	 * OtpNodeStatus} handler object contains callback methods, that will be called
	 ...
	 *
	 * <p>
	 * Only a single attempt is made to connect to the remote node, so for example
	 * it is not possible to specify an extremely long timeout and expect to be
	 * notified when the node eventually comes up. If you wish to wait for a remote
	 * node to be started, the following construction may be useful:
	 * </p>
	 *
	 * <pre>
	 ...
	 *
	 * @param timeout
	 *            the time, in milliseconds, to wait for response before returning
	 *            false.
	 *
	...
	public boolean ping(final String anode, final long timeout) {
		if (anode.equals(node)) {
			return true;
		} else if (anode.indexOf('@', 0) < 0 && anode.equals(node.substring(0, node.indexOf('@', 0)))) {
			return true;
		}
	...
	private synchronized void remoteStatus(final String anode, final boolean up, final Object info) {
		if (handler == null) {
			return;
		}
		try {
	...
	synchronized void localStatus(final String anode, final boolean up, final Object info) {
		if (handler == null) {
			return;
		}
		try {
	...
	synchronized void connAttempt(final String anode, final boolean incoming, final Object info) {
		if (handler == null) {
			return;
		}
		try {
