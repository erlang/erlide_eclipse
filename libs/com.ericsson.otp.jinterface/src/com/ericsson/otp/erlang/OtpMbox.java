...
public class OtpMbox {
	 ...
	 *
	 * <p>
	 * The {@link OtpErlangPid pid} associated with this mailbox uniquely identifies
	 * the mailbox and can be used to address the mailbox. You can send the
	 * {@link OtpErlangPid pid} to a remote communicating part so that he can know
	 * where to send his response.
	 * </p>
	 *
	 ...
	 * enables others to send messages without knowing the {@link OtpErlangPid pid}
	 * of the mailbox. A mailbox can have at most one name; if the mailbox already
	 * had a name, calling this method will supercede that name.
	 * </p>
	 *
	 * @param aname
	 *            the name to register for the mailbox. Specify null to unregister
	 *            the existing name from this mailbox.
	 *
	 ...
	 *
	 * @return an {@link OtpErlangObject OtpErlangObject} representing the body of
	 *         the next message waiting in this mailbox.
	 *
	 * @exception OtpErlangDecodeException
	 *                if the message can not be decoded.
	 *
	 * @exception OtpErlangExit
	 *                if a linked {@link OtpErlangPid pid} has exited or has sent an
	 *                exit signal to this mailbox.
	 */
	 ...
	 *
	 * @param timeout
	 *            the time, in milliseconds, to wait for a message before returning
	 *            null.
	 *
	 * @return an {@link OtpErlangObject OtpErlangObject} representing the body of
	 *         the next message waiting in this mailbox.
	 ...
	 *                exit signal to this mailbox.
	 */
	public OtpErlangObject receive(final long timeout) throws OtpErlangExit, OtpErlangDecodeException {
		try {
	 ...
	 *                available.
	 */
	public OtpInputStream receiveBuf(final long timeout) throws InterruptedException, OtpErlangExit {
		final OtpMsg m = receiveMsg(timeout);
		if (m != null) {
			return m.getMsgBuf();
		}
	 ...
	 * Block until a message arrives for this mailbox.
	 *
	 * @return an {@link OtpMsg OtpMsg} containing the header information as well as
	 ...
	 *                available.
	 */
	public OtpMsg receiveMsg(final long timeout) throws InterruptedException, OtpErlangExit {
	 ...
	 *
	 * @param to
	 *            the {@link OtpErlangPid pid} identifying the intended recipient of
	 *            the message.
	 *
	 * @param msg
	 ...
	 *
	 * <p>
	 * After this operation, the mailbox will no longer be able to receive messages.
	 * Any delivered but as yet unretrieved messages can still be retrieved however.
	 * </p>
	 *
	 * <p>
	 * If there are links from this mailbox to other {@link OtpErlangPid pids}, they
	 * will be broken when this method is called and exit signals will be sent.
	 * </p>
	 *
	 * @param reason
	...
	/**
	 * <p>
	 * Send an exit signal to a remote {@link OtpErlangPid pid}. This method does
	 * not cause any links to be broken, except indirectly if the remote
	 * {@link OtpErlangPid pid} exits as a result of this exit signal.
	 * </p>
	 *
	 * @param to
	 *            the {@link OtpErlangPid pid} to which the exit signal should be
	 *            sent.
	 *
	 * @param reason
	 ...
	 * subsequent attempt to retrieve a message through this mailbox will cause an
	 ...
	 *
	 * @exception OtpErlangExit
	 *                if the {@link OtpErlangPid pid} referred to does not exist or
	...
	/**
	 * <p>
	 * Remove a link to a remote mailbox or Erlang process. This method removes a
	 * link created with {@link #link link()}. Links are idempotent; calling this
	 * method once will remove all links between this mailbox and the remote
	 * {@link OtpErlangPid pid}.
	 * </p>
	 *
	 * @param to
	 ...
	 * connection, since connections are created automatically first time a message
	 * is sent to a {@link OtpErlangPid pid} on the remote node.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This method calls a method with the same name in {@link OtpNode#ping Otpnode}
	...
	/**
	 * <p>
	 * Get a list of all known registered names on the same {@link OtpNode node} as
	 * this mailbox.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Determine the {@link OtpErlangPid pid} corresponding to a registered name on
	 ...
	 * </p>
	 *
	 * @return the {@link OtpErlangPid pid} corresponding to the registered name, or
	 *         null if the name is not known on this node.
	 */
	 ...
	 * <p>
	 * If there are links from this mailbox to other {@link OtpErlangPid pids}, they
	 * will be broken when this method is called and exit signals with reason
	 * 'normal' will be sent.
	 * </p>
	 *
	 * <p>
