...
public abstract class AbstractBackend implements IBackend {
	...
	protected void doConnect(String label) {
		...
		try {
			...
			fNode = new OtpNode(BackendManager
					...
					.retrieveCookie());
			System.out.println("java node is " + fNode.node());

			fPeer = new OtpPeer(BackendManager.buildNodeName(label));
			System.out.println("erlang peer is " + label + " ("
	 ...
	 */
	public void dispose() {
		System.out.println("disposing backend " + fLabel);
	...
	protected void handleReceiveEvent() {
		...
		// if (msg == null)
		// continue;
		// System.out.println("handleReceiveEvent() - Event! "
		// + msg.toString());
		...
		// }
		// }
		// // System.out.println("exited event thread");
		// }
		// catch (OtpErlangExit e)
		// {
		// System.out.println("Erlide backend: event source crashed.");
		// }
	...
	private RpcResult sendRpc(String module, String fun,
		...
		try {
			...
			if (res == null) {
				if (CHECK_RPC) {
					System.out.println("    timed out: " + m + ":" + f + "("
							+ a + ")");
				}
				return null;
			}
