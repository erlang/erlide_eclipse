...
public class DuplicatesView extends ViewPart {
	...
	ISelectionListener listener = new ISelectionListener() {
		...
		public void selectionChanged(final IWorkbenchPart part, final ISelection selection) {
			MessageDialog.openInformation(getSite().getShell(), "test", selection.toString());
	...
	private final Action copyFunCallToClipboard = new ClipboardAction(PlatformUI.getWorkbench().getDisplay()) {
		...
		public void run() {
			if (!hasText) {
				MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						"Empty clipboard", "There is no FunCall element according the selected item!");
			} else {
				super.run();
			}
		}
	...
	private void makeActions() {
		...
		copyGeneralisedToClipboard.setImageDescriptor(
				PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_COPY));
		copyFunCallToClipboard.setToolTipText("Copy FunCall to the clipboard");
		copyFunCallToClipboard.setImageDescriptor(
				PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_COPY));
