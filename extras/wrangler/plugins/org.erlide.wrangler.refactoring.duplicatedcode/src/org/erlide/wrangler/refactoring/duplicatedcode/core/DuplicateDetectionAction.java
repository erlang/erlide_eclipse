...
public class DuplicateDetectionAction extends AbstractDuplicatesSearcherAction {
	...
	protected IResultParser callRefactoring()
		...
		if (onlyInfile) {
			...
			result = backend.callWithoutParser(WranglerRefactoringBackend.UNLIMITED_TIMEOUT, functionName, "xiiis", fpl,
					minToks, minClones, GlobalParameters.getTabWidth(), suffixPath);
		} else {
			functionName = "duplicated_code_eclipse";
			result = backend.callWithoutParser(WranglerRefactoringBackend.UNLIMITED_TIMEOUT, functionName, "xiiis",
					sel.getSearchPath(), minToks, minClones, GlobalParameters.getTabWidth(), suffixPath);
		}
	...
	private String getSuffixPath() throws IOException {
		final Bundle[] bs = Platform
				.getFragments(Platform.getBundle(org.erlide.wrangler.refactoring.Activator.PLUGIN_ID));
		if (bs.length < 1) {
	...
	protected boolean getUserInput() {
		...
		final DuplicateCodeDetectionInputDialog inputd = new DuplicateCodeDetectionInputDialog(shell,
