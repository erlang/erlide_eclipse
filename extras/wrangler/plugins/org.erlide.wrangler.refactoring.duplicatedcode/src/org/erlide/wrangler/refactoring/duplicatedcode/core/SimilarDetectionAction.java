...
public class SimilarDetectionAction extends AbstractDuplicatesSearcherAction {
	...
	protected IResultParser callRefactoring()
		...
		if (onlyInFile) {
			...
			result = backend.callWithoutParser(WranglerRefactoringBackend.UNLIMITED_TIMEOUT, functionName, "xiiiidxi",
					fpl, minLen, minToks, minFreq, maxNewVars, simScore, sel.getSearchPath(),
					GlobalParameters.getTabWidth());
		} else {
			result = backend.callWithoutParser(WranglerRefactoringBackend.UNLIMITED_TIMEOUT, functionName, "xiiiidxi",
					sel.getSearchPath(), minLen, minToks, minFreq, maxNewVars, simScore, sel.getSearchPath(),
					GlobalParameters.getTabWidth());
		}
	...
	protected boolean getUserInput() {
		...
		final SimilarCodeDetectionInputDialog inputd = new SimilarCodeDetectionInputDialog(shell,
