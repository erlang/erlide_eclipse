/*******************************************************************************
 * Copyright (c) 2000, 2001, 2005 IBM Corporation and others.
...
public class ProcessClosure {
	...
	protected static class ReaderThread extends Thread {
		 ...
		 */
		public ReaderThread(ThreadGroup group, String name, InputStream in,
				OutputStream out) {
			super(group, name);
			fOutputStream = out;
			fInputStream = in;
	 ...
	 *            <code>null</code>, if not interested in reading the output
	 */
	public ProcessClosure(Process process, OutputStream outputStream,
	...
	public void runNonBlocking() {
		final ThreadGroup group = new ThreadGroup("CBuilder" + fCounter++); //$NON-NLS-1$

		final InputStream stdin = fProcess.getInputStream();
		final InputStream stderr = fProcess.getErrorStream();
