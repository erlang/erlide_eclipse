...
public class BuildConsolePartitioner implements IDocumentPartitioner,
		IDocumentPartitionerExtension, IConsole, IPropertyChangeListener {
	...
	public void appendToDocument(String text, BuildConsoleStream stream) {
			...
			public void run() {
				...
				try {
					...
					if (fLastStream == null) {
						...
						fDocument.replace(fDocument.getLength(), 0, entry
								.getText());
						checkOverflow();
					}
		...
		final Display display = ErlideErlcPlugin.getStandardDisplay();
	...
	void warnOfContentChange(BuildConsoleStream stream) {
		if (stream != null) {
			ConsolePlugin.getDefault().getConsoleManager().warnOfContentChange(
					stream.getConsole());
		}
	...
	public IRegion documentChanged2(DocumentEvent event) {
		...
		for (int i = 1; i < affectedRegions.length; i++) {
			final ITypedRegion region = affectedRegions[i];
	...
	protected void checkOverflow() {
		if (fMaxLines >= 0) {
			final int nLines = fDocument.getNumberOfLines();
			if (nLines > fMaxLines + 1) {
				...
				while (partitions.hasNext()) {
					...
					if (region instanceof BuildConsolePartition) {
						...
						if (offset < overflow) {
							final int endOffset = offset + region.getLength();
							if (endOffset < overflow) {
								// remove partition
							} else {
								// split partition
								final int length = endOffset - overflow;
	...
	private BuildConsolePartition addPartition(BuildConsolePartition partition) {
		if (fPartitions.isEmpty()) {
			...
			final int index = fPartitions.size() - 1;
			final BuildConsolePartition last = (BuildConsolePartition) fPartitions
					.get(index);
	...
	public ConsoleOutputStream getOutputStream() throws CoreException {
		return new BuildOutputStream(fManager
				.getStream(BuildConsoleManager.BUILD_STREAM_TYPE_OUTPUT));
	}

	public ConsoleOutputStream getInfoStream() throws CoreException {
		return new BuildOutputStream(fManager
				.getStream(BuildConsoleManager.BUILD_STREAM_TYPE_INFO));
	}

	public ConsoleOutputStream getErrorStream() throws CoreException {
		return new BuildOutputStream(fManager
				.getStream(BuildConsoleManager.BUILD_STREAM_TYPE_ERROR));
	}
