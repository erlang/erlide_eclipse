...
public class BuildConsoleViewer extends TextViewer implements LineStyleListener {
	...
	/**
	 * Creates a new console viewer and adds verification checking to only allow
	 * text modification if the text is being modified in the editable portion
	...
	private static int getSWTStyles() {
		final int styles = SWT.H_SCROLL | SWT.V_SCROLL;
		return styles;
	}
	...
	protected void revealEndOfDocument() {
		if (isAutoScroll()) {
			...
			try {
				// lines are 0-based
				final int lineStartOffset = doc.getLineOffset(lines - 1);
	...
	public void setDocument(IDocument doc) {
		final IDocument oldDoc = getDocument();
	...
	public void lineGetStyle(LineStyleEvent event) {
		...
		if (document != null) {
			final BuildConsolePartitioner partitioner = (BuildConsolePartitioner) document
			...
			if (partitioner != null) {
				...
				for (int i = 0; i < regions.length; i++) {
					...
					final Color color = partition.getStream().getColor();
					styles[i] = new StyleRange(partition.getOffset(), partition
							.getLength(), color, null);
				}
				event.styles = styles;
			}
		}
