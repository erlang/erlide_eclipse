...
public class ErrorPattern {
	 ...
	 * @param severity
	 */
	public ErrorPattern(String pattern, int groupFileName, int groupLineNum,
			int groupDesc, int groupVarName, int severity) {
	...
	/**
	 * Pattern for errors not associated file a file (e.g. make and linker
	 * errors).
	 * 
	 * @param pattern
	 * @param groupDesc
	...
	public int getLineNum(Matcher matcher) {
		try {
			return groupLineNum != 0 ? Integer.valueOf(
					matcher.group(groupLineNum)).intValue() : 0;
	...
	protected boolean recordError(Matcher matcher, ErrorParserManager eoParser) {
		final int severity = getSeverity(matcher);
		if (severity == -1) {
			// Skip
			return true;
		}

		final String fileName = getFileName(matcher);
		final int lineNum = getLineNum(matcher);
		String desc = getDesc(matcher);
		final String varName = getVarName(matcher);
