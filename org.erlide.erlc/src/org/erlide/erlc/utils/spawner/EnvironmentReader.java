 ...
 *******************************************************************************/
package org.erlide.erlc.utils.spawner;
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (idx != -1) {
					...
					if (isWin32) { // Since windows env ignores case let
						// normalize to Upper here.
						key = key.toUpperCase();
					}
					final String value = line.substring(idx + 1);
					envVars.setProperty(key, value);
					// Save key and value in case we're dealing with a variable
					// with
					...
					// with a \n replaced in between, and saving the new value
					// with
					// the previous key.
					if (prev_key != null) {
						if (prev_value != null) {
