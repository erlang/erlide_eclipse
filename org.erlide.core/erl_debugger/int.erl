...
action_at_break(_, _, _) -> nyi().
test_at_break(_, _, _) -> nyi().
get_binding(_Var, _Bs) -> nyi().
...
nyi() -> 
    io:format("unavailable from within ErlIDE, use Eclipse debugger functions instead\n"), 
    {error, unavaible}.
