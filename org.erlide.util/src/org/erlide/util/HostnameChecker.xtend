			...
			val Job job = new Job("") {
				...
				override protected run(IProgressMonitor monitor) {
					if (Platform.getBundle("org.erlide.ui").getState() !== Bundle.ACTIVE) {
						...
						You use the system properties erlide.long.name or erlide.short.name to set the host names to be used by both Erlang and Java.
						
						The new way to do that is to edit ~/.erlide.hosts and change the values there if they aren't correct. 
						Remove the use of the system properties.''')
					}
